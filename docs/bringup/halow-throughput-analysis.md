...
delivery, body goodput, and diagnostic flags. `--events-dir` recomputes saved
stage metrics from `CASE.events.jsonl` using the runner's assessor and reports
any discrepancy. Adjacent comparison deltas require the same direction, width,
