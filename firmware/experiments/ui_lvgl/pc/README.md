...
Webデザイン13〜16の再現は次で起動する。画面内の左スワイプ（マウスで左へドラッグ）で13→14→15→16→13、右スワイプで逆順に戻る。詳しい設計と検証は [DOT_STUDIES.md](DOT_STUDIES.md)。
...
| `4` / `5` / `6` / `7` | Web Dot 13 / 14 / 15 / 16（初期の受信状態に戻す） |
| Dot画面で左右ドラッグ | 左＝次、右＝前。4画面を循環。短い移動・縦／斜め・1秒超の操作は無視 |
| Dot画面で `G` / `X` / `T` / `B` | 同じ画面で待受 / RX / TX / 空き待ち |
| Dot画面で `D` / `L` / `M` | 位置未更新 / 通信断 / 受信ミュート切替 |
...
28テストを実行する（従来16件＋Dot画像11件＋Dot操作1件）。
...
- **Dot画像11種類**：13〜16の4画面、待受・TX・空き待ち・位置未更新・通信断・ミュート・5桁距離。
- **Dot操作**：左右の巡回、短距離・縦・斜め・長押しの抑止、キャンセル、状態保持、古い距離の非表示、波形の停止／再開。SDLテストにも等倍・2倍・3倍の左右巡回を追加。
